...
double* append_to_array(double element,
                        ...
                        int &current_size,
                        int &max_size) {
  double* res = new double[max_size];
  if(current_size == max_size){
    res = extend_array(array,max_size,max_size+5);
    max_size = max_size +5;
...
double* remove_from_array(double* array,
  ...
  if(current_size + 5 <= max_size){
    res = shrink_array(array,max_size,max_size-5);
    max_size = max_size - 5;
    return res;
  }else{
    return array;
...
void merge_telemetry(double **telemetries,
  ...
  for(int i = 0; i<10*global_telemetry_current_size; i++){
    for(int j = 0; j<(global_telemetry_current_size/3)-1; j++){
